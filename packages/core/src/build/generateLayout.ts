...
export async function generateFolderComparison(options: GenerateFolderComparisonOptions): Promise<GeneratedLayout> {
  ...
  const right = path.resolve(options.right);
  const cap = options.nPagesMax ?? DEFAULT_PAGES_MAX;
  const resolution = options.resolution ?? 150;

  const [leftFiles, rightFiles] = await Promise.all([
    findPlotFiles(left, options.exclude),
    findPlotFiles(right, options.exclude),
  ]);

  const everything = [...new Set([...leftFiles, ...rightFiles])].sort((a, b) => a.localeCompare(b));
  const columns = ['Description', path.basename(left), path.basename(right), 'Difference'];
  const rows: string[][] = [];
  const files: DiscoveredFile[] = [];
  ...
  for (const relativePath of everything) {
    ...
    for (let page = 1; page <= pages; page += 1) {
      ...
      const rightSpec = inRight ? spec(options.layoutDir, path.join(right, relativePath), page, resolution) : '';

      const difference = !inLeft
        ? `Only in ${path.basename(right)}::center`
        : !inRight
          ? `Only in ${path.basename(left)}::center`
          : `diff(\`${columns[1]}\`, \`${columns[2]}\`)`;
  ...
  return {
    layout: {
      ...
      comments: ['# Folder comparison generated by plotExcel — files are paired by their path inside each folder.'],
      columns,
      rows,
    },
    files,
...
function spec(layoutDir: string, absolutePath: string, page: number, resolution: number): string {
  return `${toPosix(path.relative(layoutDir, absolutePath))}::page ${page}::resolution ${resolution}`;
}
